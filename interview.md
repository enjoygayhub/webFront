...
### setState后发生了什么
调用 setState 后，React 会创建更新对象并加入当前组件的 Fiber 更新队列，进入调度系统；随后在 Render 阶段进行 Fiber 树的构建与 Diff 算法对比，标记 DOM 变更；最后在 Commit 阶段统一执行真实 DOM 更新、执行副作用与回调，完成视图渲染。整个过程在合成事件与生命周期中是异步批量处理的。
