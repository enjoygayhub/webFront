...
> <font color='red'>（注意大写字母开头
> ES6 中新增的 Symbol 类型，代表创建后独一无二且不可变的数据类型，主要是为了解决可能出现的全局变量冲突的问题。
>
> ES2020新增BigInt，用于处理任意精度整数的新数字基元n，比如3n。


> + **引用类型**是指Object即广义的对象类型复杂类型。可由多个简单类型的值的合成，可以看作是一个存放各种值的容器。包括Array,Math，Date,RegExp,Function等
...
- undefined 代表的含义是未定义，null 代表的含义是空。一般变量声明了但还没有定义的时候会返回 undefined，null主要用于赋值给一些可能会返回对象的变量，作为初始化。
...
> 返回一个数组,对象自身的(不含继承的)所有可枚举属性.数组属性名的排列顺序和正常循环遍历该对象时返回的顺序一致 。
...
console.log(obj.diameter()) // 20 diameter是普通函数，里面的this指向直接调用它的对象obj。
console.log(obj.perimeter()) // NaN 相当于在全局window下执行箭头函数
...
Ajax 即“Asynchronous Javascript And XML”（异步 JavaScript 和 XML）， js 脚本向服务器发起 http 通信，然后根据服务器返回的数据。
