...
Unicode 码点：字符的唯一 ID，逻辑层面；
UTF-16：JS 字符串物理存储编码，用 1~2 个 16 位单元存码点；
旧 API 比如charCodeAt只算 UTF-16 单元，所以 emoji / 生僻汉字会 length 异常；
现代用码点相关 API，才是真正操作 Unicode 字符。

### 值和对象不同，比如 3 和 new Number(3).那为啥对象的方法在基本类型上
运行时装箱了
 比如console.log("abc".charAt(0)); //a
...
console.log(o.a); // 1
访问器属性跟数据属性不同，每次访问属性都会执行getter或者setter函数。这里我们的getter函数返回了1，所
以o.a每次都得到1。
